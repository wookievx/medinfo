...
import pl.edu.agh.llampart.medinfo.medinfo.{DateTime, LedgerGrpc, Request, Response}

class PatientClient
object PatientClient extends App{
  private val logger = Logger.getLogger(classOf[PatientClient].getSimpleName)
  ...
  case class Query(name: String = "", types: Seq[String] = Seq.empty,
  ...
  private val parser = new scopt.OptionParser[Query]("find") {
    ...
    opt[String]('i', "id").action((id, q) => q.copy(name = id)).text("Your identification")
  ...
  Iterator.continually {
    parser.parse(scala.io.StdIn.readLine("Type your query\n").split("""\s+"""), Query()) match {
      case Some(q) if q.exit =>
        channel.shutdown()
        false
      case Some(Query(name, types, from, to, false)) =>
        val fromDt = from.map(c => DateTime(c.getTimeInMillis))
        val toDt = to.map(c => DateTime(c.getTimeInMillis))
        stub.requestExamination(Request(types, None, Some(name), fromDt, toDt), new StreamObserver[Response] {
